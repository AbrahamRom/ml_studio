...
from ml_pipeline.classic_ml import CLASSIC_MODELS, get_available_models, train_classic_model, optimize_classic_hyperparameters, HPO_SEARCH_SPACES, DEFAULT_PARAMS
from ml_pipeline.automl_runner import run_target_automl, FULL_MLJAR_ALGORITHMS
from utils.pagination import paginated_dataframe

warnings.filterwarnings("ignore")

st.markdown("# 🎨 Entrenamiento Visual")
st.markdown('<div class="section-header">Configuración interactiva de modelos</div>', unsafe_allow_html=True)
...
tab_dl, tab_classic, tab_automl = st.tabs(["🧠 Deep Learning", "📊 Modelos Clásicos", "🤖 AutoML"])
...
with tab_classic:
    ...
    if not available:
        st.warning("No hay modelos clásicos disponibles. Instala lightgbm, xgboost y/o catboost.")
        st.stop()

    classic_name = st.selectbox("Algoritmo", available, key="classic_algo")

    st.markdown("### 2️⃣ Configurar datos")

    num_cols = df.select_dtypes(include=[np.number]).columns.tolist()
    default_features = [c for c in num_cols if c not in ["real_rend"]]
    feature_cols = st.multiselect("Columnas de entrada (features)", options=df.columns.tolist(), default=default_features[:min(8, len(default_features))], key="classic_features")

    target_options = [c for c in df.columns if c not in feature_cols]
    target_col = st.selectbox("Columna objetivo (target)", options=target_options, key="classic_target")

    if target_col is not None:
        unique_vals = df[target_col].dropna().unique()
        is_class = len(unique_vals) <= 10 and all(isinstance(v, (int, float, bool, str)) for v in unique_vals)
        task = st.selectbox("Tipo de tarea", ["classification", "regression"], index=0 if is_class else 1, key="classic_task")
    else:
        task = "classification"

    st.markdown("### 3️⃣ Hiperparámetros")

    defaults = DEFAULT_PARAMS.get(classic_name, {})
    search_space = HPO_SEARCH_SPACES.get(classic_name, {})

    def init_classic_hp_state():
        for param, default_val in defaults.items():
            key = f"_chp_{param}"
            if key in st.session_state:
                del st.session_state[key]
            st.session_state[key] = default_val
        st.session_state.classic_prev_algo = classic_name

    if "classic_prev_algo" not in st.session_state or st.session_state.classic_prev_algo != classic_name:
        init_classic_hp_state()

    if st.session_state.get("classic_hpo_applied"):
        hpo_config = st.session_state.classic_hpo_applied
        for param, val in hpo_config.items():
            key = f"_chp_{param}"
            st.session_state[key] = val
        del st.session_state.classic_hpo_applied
        st.session_state.classic_hpo_done = True
        st.rerun()

    if st.session_state.get("classic_hpo_done"):
        st.info("✅ Hiperparámetros actualizados con la mejor configuración encontrada.")
        if st.button("🔄 Usar defaults", key="_chp_reset_btn"):
            init_classic_hp_state()
            if "classic_hpo_done" in st.session_state:
                del st.session_state.classic_hpo_done
            st.rerun()

    if search_space:
        param_cols = min(len(search_space), 5)
        param_rows = [list(search_space.keys())[i:i+param_cols] for i in range(0, len(search_space), param_cols)]
        for row_params in param_rows:
            cols = st.columns(len(row_params))
            for col_w, param in zip(cols, row_params):
                options = search_space[param]
                state_key = f"_chp_{param}"
                current_val = st.session_state.get(state_key, options[0])
                with col_w:
                    if None in options:
                        str_options = ["None" if v is None else v for v in options]
                        current_str = "None" if current_val is None else current_val
                        idx = str_options.index(current_str) if current_str in str_options else 0
                        st.selectbox(param, options=str_options, index=idx, key=state_key)
                    elif isinstance(options[0], tuple):
                        str_options = [str(v) for v in options]
                        current_str = str(current_val)
                        idx = str_options.index(current_str) if current_str in str_options else 0
                        st.selectbox(param, options=str_options, index=idx, key=state_key)
                    else:
                        idx = options.index(current_val) if current_val in options else 0
                        st.selectbox(param, options=options, index=idx, key=state_key)
    else:
        st.caption("Este modelo no tiene hiperparámetros configurables.")

    if search_space:
        col_reset, _ = st.columns([1, 4])
        with col_reset:
            if st.button("🔄 Resetear a defaults", key="classic_reset_hp"):
                init_classic_hp_state()
                st.rerun()

    hpo_params = {}
    for param in search_space:
        hpo_params[param] = st.session_state.get(f"_chp_{param}")

    if st.session_state.get("classic_hpo_result"):
        hpo_result = st.session_state.classic_hpo_result
        st.divider()
        st.markdown("### 🏆 Mejor configuración (HPO)")
        if hpo_result["best_config"]:
            st.json(hpo_result["best_config"])
        ...
        if valid_trials:
            ...
            fig.add_trace(go.Scatter(
                ...
                y=[t["val_score"] if is_class else t["val_rmse"] for t in valid_trials],
                mode="lines+markers",
                line=dict(color="#2dd4bf"),
                marker=dict(size=8),
                name="Val Score" if is_class else "Val RMSE"
            ))
            fig.update_layout(
                title="HPO: Métrica por trial",
                xaxis_title="Trial",
                yaxis_title="Val Score" if is_class else "Val RMSE",
    ...
    st.markdown("### 4️⃣ Entrenar / Optimizar")

    col_a, col_b = st.columns(2)
    with col_a:
        test_size = st.slider("Validation size", 0.1, 0.4, 0.2, 0.05, key="classic_test")
    with col_b:
        random_state = st.number_input("Random state", min_value=0, max_value=9999, value=42, step=1, key="classic_rs")

    col_btn1, col_btn2 = st.columns(2)

    with col_btn1:
        if st.button("🚀 Entrenar", type="primary", use_container_width=True, key="classic_train_btn"):
            if not feature_cols:
                st.error("Selecciona al menos una columna de entrada.")
            elif target_col is None:
                st.error("Selecciona una columna objetivo.")
            else:
                try:
                    progress_bar = st.progress(0, text="Entrenando...")

                    def progress_cb(epoch, total, _, __):
                        progress_bar.progress(epoch / total, text=f"Epoch {epoch}/{total}")

                    result = train_classic_model(df, feature_cols, target_col, classic_name, task=task, test_size=float(test_size), random_state=int(random_state), params=hpo_params, progress_callback=progress_cb)
                    progress_bar.progress(1.0, text="Completado!")

                    st.success(f"✅ {classic_name} entrenado exitosamente.")
                    if "classic_results" not in st.session_state:
                        st.session_state.classic_results = []
                    st.session_state.classic_results.append(result)
                    st.rerun()
                except Exception as e:
                    st.error(f"Error al entrenar: {e}")
                    import traceback
    ...
    with col_btn2:
        if st.button("🔍 Optimizar hiperparámetros", use_container_width=True, key="classic_hpo_btn"):
            if not feature_cols:
                st.error("Selecciona al menos una columna de entrada.")
            elif target_col is None:
                st.error("Selecciona una columna objetivo.")
                ...
                try:
                    progress_bar = st.progress(0, text="Optimizando...")
                    trial_info = st.empty()

                    def hpo_cb(trial, total):
                        progress_bar.progress(trial / total, text=f"Trial {trial}/{total}")
                        trial_info.caption(f"Completado {trial} de {total}")

                    hpo_result = optimize_classic_hyperparameters(df, feature_cols, target_col, classic_name, task=task, test_size=float(test_size), random_state=int(random_state), n_trials=30, progress_callback=hpo_cb)

                    progress_bar.progress(1.0, text="Búsqueda completada!")

                    if hpo_result.get("message"):
                        st.warning(hpo_result["message"])
                    elif hpo_result["best_config"]:
                        st.success(f"✅ Mejor Val Score: {hpo_result['best_val_score']:.4f}")
                        st.session_state.classic_hpo_applied = hpo_result["best_config"]
                        st.session_state.classic_hpo_result = hpo_result
                        st.rerun()
                    else:
                        st.warning("No se encontró una configuración válida.")
    ...
    if st.session_state.get("classic_results"):
        last_result = st.session_state.classic_results[-1]
        st.divider()
        st.markdown(f"### 📊 Métricas: {last_result['model_name']}")

        if last_result["task"] == "classification":
            r1, r2, r3, r4, r5, r6 = st.columns(6)
            r1.metric("Accuracy (Val)", f"{last_result['val_score']:.4f}")
            r2.metric("Precision (Val)", f"{last_result['val_precision']:.4f}")
            r3.metric("Recall (Val)", f"{last_result['val_recall']:.4f}")
            r4.metric("F1 (Val)", f"{last_result['val_f1']:.4f}")
            r5.metric("RMSE (Val)", f"{last_result['val_rmse']:.4f}")
            r6.metric("MAE (Val)", f"{last_result['val_mae']:.4f}")
            r7, r8, r9, r10, r11, r12 = st.columns(6)
            r7.metric("Accuracy (Train)", f"{last_result['train_score']:.4f}")
            r8.metric("Precision (Train)", f"{last_result['train_precision']:.4f}")
            r9.metric("Recall (Train)", f"{last_result['train_recall']:.4f}")
            r10.metric("F1 (Train)", f"{last_result['train_f1']:.4f}")
            r11.metric("RMSE (Train)", f"{last_result['train_rmse']:.4f}")
            r12.metric("MAE (Train)", f"{last_result['train_mae']:.4f}")
        else:
            r1, r2, r3, r4, r5, r6 = st.columns(6)
            r1.metric("R² (Val)", f"{last_result['val_score']:.4f}")
            r2.metric("RMSE (Val)", f"{last_result['val_rmse']:.4f}")
            r3.metric("MSE (Val)", f"{last_result['val_mse']:.4f}")
            r4.metric("MAE (Val)", f"{last_result['val_mae']:.4f}")
            r5.metric("R² (Train)", f"{last_result['train_score']:.4f}")
            r6.metric("RMSE (Train)", f"{last_result['train_rmse']:.4f}")

        st.divider()
        st.markdown("### 🧪 Probar Modelo")

        max_rows = min(1000, len(df))
        selected_idx = st.slider("Índice de fila", 0, max_rows - 1, 0, key="classic_test_idx")

        model_feature_cols = last_result["feature_cols"]
        test_row = df.iloc[selected_idx:selected_idx+1][model_feature_cols].copy()
        st.markdown("#### Datos de entrada (editables)")
        edited_row = st.data_editor(test_row, num_rows="fixed", key="classic_test_editor", use_container_width=True)

        if st.button("🚀 Probar Modelo", type="primary", use_container_width=True, key="classic_test_btn"):
            model = last_result["model"]
            scaler = last_result["scaler"]
            le = last_result.get("label_encoder")

            row_vals = edited_row[model_feature_cols].values.astype(float)
            row_vals = np.nan_to_num(row_vals, nan=0.0)
            row_scaled = scaler.transform(row_vals)
            ...
            if le:
                ...
                st.markdown("#### Resultado")
                st.metric("Predicción", str(pred_label))
                st.code(f"Valor numérico: {pred}")
            else:
                st.markdown("#### Resultado")
                st.metric("Predicción", f"{pred:.4f}")

        st.divider()
        st.markdown("### 💾 Guardar Modelo")
        st.caption("Guarda el modelo para usarlo en **Test Models**.")

        model_save_name = st.text_input("Nombre del modelo", value=f"{classic_name}_{len(st.session_state.saved_classic_models)+1}", key="classic_save_name")

        col_save1, col_save2 = st.columns([1, 2])
        with col_save1:
            if st.button("💾 Guardar", type="primary", use_container_width=True, key="classic_save_btn"):
                if model_save_name:
                    if model_save_name in st.session_state.saved_classic_models:
                        st.warning(f"Ya existe '{model_save_name}'. Se sobrescribirá.")

                    st.session_state.saved_classic_models[model_save_name] = {
                        "name": model_save_name,
                        "type": "classic",
                        "model_name": classic_name,
                        "model": last_result["model"],
                        "scaler": last_result["scaler"],
                        "label_encoder": last_result.get("label_encoder"),
                        "feature_cols": model_feature_cols,
                        "target_col": last_result["target_col"],
                        "task": last_result["task"],
                        "metadata": {
                            "train_score": last_result["train_score"],
                            "val_score": last_result["val_score"],
                            "train_rmse": last_result["train_rmse"],
                            "val_rmse": last_result["val_rmse"],
                            "params": last_result.get("params"),
                        },
                        "timestamp": pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"),
                    }
                    st.success(f"✅ Modelo '{model_save_name}' guardado.")
                    st.rerun()

        with col_save2:
            if st.session_state.saved_classic_models:
                st.markdown("**Modelos guardados:**")
                for name, info in st.session_state.saved_classic_models.items():
                    st.caption(f"• `{name}` ({info['model_name']}) - {info['timestamp']}")

with tab_automl:
    if "saved_automl_models" not in st.session_state:
        st.session_state.saved_automl_models = {}

    st.markdown("### 1️⃣ Configurar datos")

    num_cols = df.select_dtypes(include=[np.number]).columns.tolist()
    default_features = [c for c in num_cols if c not in ["real_rend"]]
    automl_features = st.multiselect("Columnas de entrada (features)", options=df.columns.tolist(), default=default_features[:min(8, len(default_features))], key="automl_features")

    target_options = [c for c in df.columns if c not in automl_features]
    automl_target = st.selectbox("Columna objetivo (target)", options=target_options, key="automl_target")

    if automl_target is not None:
        unique_vals = df[automl_target].dropna().unique()
        is_class = len(unique_vals) <= 10 and all(isinstance(v, (int, float, bool, str)) for v in unique_vals)
        automl_task = "classification" if is_class else "regression"
    else:
        automl_task = "classification"

    st.markdown("### 2️⃣ Configuración de AutoML")

    col_a1, col_a2, col_a3 = st.columns(3)
    with col_a1:
        automl_mode = st.selectbox("Modo", ["Perform", "Explain", "Compete"], index=1, key="automl_mode")
    with col_a2:
        automl_time = st.slider("Tiempo límite (segundos)", 30, 600, 180, 10, key="automl_time")
    with col_a3:
        automl_rs = st.number_input("Random state", min_value=0, max_value=9999, value=42, step=1, key="automl_rs")

    metric_options = ["accuracy", "f1", "roc_auc", "r2", "rmse", "mae", "mse"]
    default_metric = "accuracy" if automl_task == "classification" else "r2"
    automl_metric = st.selectbox("Métrica principal", options=metric_options, index=metric_options.index(default_metric), key="automl_metric")

    st.markdown("### 3️⃣ Algoritmos")

    all_algo = ["Baseline", "Linear", "Decision Tree", "Random Forest", "Extra Trees", "LightGBM", "Xgboost", "CatBoost", "Neural Network", "Nearest Neighbors"]
    selected_algos = st.multiselect("Algoritmos a entrenar", options=all_algo, default=all_algo, key="automl_algos")

    if not selected_algos:
        st.warning("Selecciona al menos un algoritmo.")
        st.stop()

    st.markdown("### 4️⃣ Entrenar")

    if st.button("🚀 Entrenar AutoML", type="primary", use_container_width=True, key="automl_train_btn"):
        if not automl_features:
            st.error("Selecciona al menos una columna de entrada.")
        elif automl_target is None:
            st.error("Selecciona una columna objetivo.")
        else:
            try:
                progress_bar = st.progress(0, text="Entrenando AutoML...")
                status_text = st.empty()

                def automl_progress_cb(epoch, total, train_loss, val_loss):
                    progress_bar.progress(min(epoch / total, 1.0), text="Entrenando...")

                config = {
                    "task": automl_task,
                    "ml_task": "binary_classification" if automl_task == "classification" else "regression",
                    "primary_metric": automl_metric,
                    "direction": "max" if automl_metric in ("accuracy", "f1", "roc_auc", "r2") else "min",
                }

                result = run_target_automl(
                    df, automl_target, config,
                    all_targets=[automl_target],
                    run_path="mljar_results",
                    test_size=0.2,
                    total_time_limit=int(automl_time),
                    mode=automl_mode,
                    algorithms=selected_algos,
                    random_state=int(automl_rs),
                    n_jobs=-1,
                )

                progress_bar.progress(1.0, text="Completado!")
                st.success(f"✅ AutoML entrenado exitosamente para '{automl_target}'.")

                if "automl_results" not in st.session_state:
                    st.session_state.automl_results = []
                st.session_state.automl_results.append(result)
                st.rerun()
            except Exception as e:
                st.error(f"Error al entrenar AutoML: {e}")
                import traceback
                st.code(traceback.format_exc())

    if st.session_state.get("automl_results"):
        last_result = st.session_state.automl_results[-1]
        st.divider()
        st.markdown("### 📊 Resultados")

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Train rows", f"{last_result['train_rows']:,}")
        c2.metric("Test rows", f"{last_result['test_rows']:,}")
        c3.metric("Mejor modelo", last_result.get("best_model_name", "-"))
        c4.metric("Mejor métrica", f"{last_result.get('best_metric_value', 0):.4f}")

        st.markdown("#### 🏆 Leaderboard")
        leaderboard = last_result.get("leaderboard")
        if leaderboard is not None and not leaderboard.empty:
            paginated_dataframe(leaderboard, key="automl_leaderboard", height=350)

        st.markdown("#### 📋 Métricas por modelo")
        per_model = last_result.get("per_model_metrics")
        if per_model is not None and not per_model.empty:
            paginated_dataframe(per_model, key="automl_per_model", height=400)

        st.divider()
        st.markdown("### 🧪 Probar Mejor Modelo")

        max_rows = min(1000, len(df))
        selected_idx = st.slider("Índice de fila", 0, max_rows - 1, 0, key="automl_test_idx")

        model_feature_cols = last_result.get("feature_cols", automl_features)
        test_row = df.iloc[selected_idx:selected_idx+1][model_feature_cols].copy()
        st.markdown("#### Datos de entrada (editables)")
        edited_row = st.data_editor(test_row, num_rows="fixed", key="automl_test_editor", use_container_width=True)

        if st.button("🚀 Probar Modelo", type="primary", use_container_width=True, key="automl_test_btn"):
            automl_obj = last_result.get("automl")
            row_data = edited_row[model_feature_cols]
            ...
            if automl_task == "classification":
                ...
                try:
                    proba = automl_obj.predict_proba(row_data)[0]
                    classes = automl_obj.classes_
                    proba_df = pd.DataFrame({"Clase": [str(c) for c in classes], "Probabilidad": proba})
                    st.dataframe(proba_df, use_container_width=True, hide_index=True)
                    fig = go.Figure()
                    fig.add_trace(go.Bar(x=proba_df["Clase"], y=proba_df["Probabilidad"], marker_color="#2dd4bf", text=proba_df["Probabilidad"].round(3), textposition="outside"))
                    fig.update_layout(title="Probabilidades", paper_bgcolor="#0d0f14", plot_bgcolor="#141720", font={"color": "#e2e8f0"}, height=300)
                    st.plotly_chart(fig, use_container_width=True)
                except Exception:
                    pass
            else:
        ...
        st.divider()
        st.markdown("### 💾 Guardar Modelos")
        st.caption("Guarda los modelos del leaderboard para usarlos en **Test Models**.")

        leaderboard = last_result.get("leaderboard")
        if leaderboard is not None and not leaderboard.empty:
            model_names = leaderboard.get("name", leaderboard.get("model_name", [])).tolist()
            if isinstance(model_names, list) and model_names:
                save_model_name = st.selectbox("Modelo a guardar", options=model_names, key="automl_save_select")
                if st.button("💾 Guardar modelo seleccionado", type="primary", use_container_width=True, key="automl_save_btn"):
                    automl_obj = last_result.get("automl")
                    st.session_state.saved_automl_models[save_model_name] = {
                        ...
                        "task": automl_task,
                        "metadata": {
                            "best_model_name": last_result.get("best_model_name"),
                            "best_metric_value": last_result.get("best_metric_value"),
                            "leaderboard": leaderboard.to_dict(),
                        },
                        "timestamp": pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"),
                    }
                    st.success(f"✅ Modelo '{save_model_name}' guardado.")
                    st.rerun()

        if st.session_state.saved_automl_models:
            st.markdown("**Modelos AutoML guardados:**")
            for name, info in st.session_state.saved_automl_models.items():
                st.caption(f"• `{name}` - {info['timestamp']}")
